...
function AlertMailConnection() {
  const interval = setInterval(() => {
    fetch(`http://77.37.51.45:3311/process-alert-mail-prevent`)
      .then((rp) => rp.json())
      .then((rsp) => {
        fetch(`http://77.37.51.45:3311/process-alert-mail-to`)
          .then((r) => r.json())
          .then((response) => {
            for (let i = 0; i < rsp.length; i++) {
              ...
              `;
                ...
                fetch(
                  `http://77.37.51.45:3311/modify-process-alert-mail-prevent`,
                  {
                    method: "PUT",
                    headers: {
